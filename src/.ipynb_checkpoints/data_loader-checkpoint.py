...
class DinDataset(torch.utils.data.Dataset):
    def __init__(self, train, image_size, class_onehot, data_path='../data/Din1', trans=None):
        'Initialization'
        if train:
...
def getDin1(batch_size, img_size=32, data_root='../data/Din1', train=True, val=True, **kwargs):
    ...
    if train:
        train_loader = torch.utils.data.DataLoader(DinDataset(True, img_size, class_onehot, data_root),
...
def getOnlyCracks(batch_size, img_size=32, data_root='../data/onlycracks', train=True, val=True, **kwargs):
    num_workers = kwargs.setdefault('num_workers', 1)
    kwargs.pop('input_size', None)
    print("Building onlycracks data loader with {} workers".format(num_workers))
    ds = []
...
def getTargetDataSet(data_type, batch_size, imageSize, dataroot):
    ...
    if data_type == 'onlycracks':
        train_loader, test_loader = getOnlyCracks(batch_size=batch_size, img_size=imageSize, data_root=dataroot, num_workers=1)
